...
# verifico la cantidad de registros por cada clase_ternaria
dtrain[, .N, clase_ternaria]
dapply[, .N, clase_ternaria]
dtrain[, .N]
dapply[, .N]

#creo la columna clase_binaria1 cumpliendo las siguientes condiciones pos = { BAJA+2} neg = { BAJA+1,CONTINUA }

dtrain[clase_ternaria == "BAJA+2", clase_binaria1 := "pos"]
dtrain[clase_ternaria != "BAJA+2", clase_binaria1 := "neg"]

# verifico la cantidad de registros por cada clase_ternaria
dtrain[, .N, clase_binaria1]
...
fwrite(dapply[, list(numero_de_cliente, Predicted)],
       file = "~/buckets/b1/exp/KA2001/K101_binaria_ranking50.csv",
        sep = ","
)
